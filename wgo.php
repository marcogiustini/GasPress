...
add_action('wp_enqueue_scripts', function () {
    wp_enqueue_style('wpgas-style', plugin_dir_url(__FILE__) . 'assets/css/wgo-style.css');
    wp_enqueue_script('wpgas-script', plugin_dir_url(__FILE__) . 'assets/js/wgo-script.js', [], false, true);
});
...
add_action('admin_init', function () {
    if (!function_exists('wc_get_product') || !function_exists('groups_get_group_members')) {
        ...
        wp_die(esc_html__('Questo plugin richiede WooCommerce e BuddyPress attivi.', 'wpgas'));
    }
});
