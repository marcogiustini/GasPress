...
function wgo_render_pickup_confirmation_button($order_id) {
    ...
    if ($confirmed) {
        echo '<p class="wgo-confirmed">' . esc_html__('Hai già confermato il ritiro.', 'GasPress-main') . '</p>';
        return;
    }
    ...
    echo '<button type="submit">' . esc_html__('Conferma ritiro', 'GasPress-main') . '</button>';
    echo '</form>';
}
